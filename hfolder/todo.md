...
* **Modify** `backend-node/server.js` — The Express app entry point (refactored to import modules).
* **Create** `backend-node/config/db.js` — MongoDB connection configuration.
* **Create** `backend-node/config/multer.js` — Multer storage configuration for uploads.
* **Create** `backend-node/utils/ip.js` — Client IP extraction utility.
* **Create** `backend-node/middleware/errorHandler.js` — Global error handler middleware.
* **Create** `backend-node/sockets/socketManager.js` — Socket.IO event handler and WebRTC signaling manager.
* **Create** `backend-node/routes/health.js` — Health check endpoint router.
* **Create** `backend-node/routes/users.js` — User authentication, discovery, and ping routes router.
* **Create** `backend-node/routes/chats.js` — User chats list, history logs, and messaging routes router.
* **Create** `backend-node/routes/files.js` — Local/cloud fallback upload and download routes router.
...
### 📁 Files to Create / Modify
* **Modify** `backend-node/sockets/socketManager.js` — Set up connection logic that authenticates the Socket.IO connection using the Clerk token.
...
* **Modify** `backend-node/routes/files.js` — Implement:
...
* **Modify** `backend-node/server.js` and `backend-node/routes/health.js` — Update health check and console messages.
